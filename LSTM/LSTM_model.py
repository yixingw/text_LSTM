...
class TextRNN:
    def __init__(self,num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,
                 vocab_size,embed_size,is_training,hidden_size,optimization_method,initializer=tf.random_normal_initializer(stddev=0.1)):
    ...
    def inference(self):
        ...
        lstm_cell=rnn.BasicLSTMCell(self.hidden_size) #forward direction cell
        ...
        outputs,_=tf.nn.dynamic_rnn(lstm_cell,self.embedded_words,dtype=tf.float32) #[batch_size,sequence_length,hidden_size] #creates a dynamic bidirectional recurrent neural network
        ...
        self.output_rnn_last=tf.reduce_mean(output_rnn,axis=1) #[batch_size,hidden_size] #output_rnn_last=output_rnn[:,-1,:] ##[batch_size,hidden_size*2] #TODO
    ...
    def train(self):
        ...
        if  self.optimization_method == "Momentum":
            train_op = tf.contrib.layers.optimize_loss(self.loss_val, global_step=self.global_step,learning_rate=learning_rate, optimizer=  tf.train.MomentumOptimizer(0.01, momentum=0.5))
        else:
            train_op = tf.contrib.layers.optimize_loss(self.loss_val, global_step=self.global_step,learning_rate=learning_rate, optimizer=  self.optimization_method)
        return train_op
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            input_x=np.load('./train_id_mat.npy')[:10] #[None, self.sequence_length]
            input_y=input_y=np.array([1,0,1,0,0,1,1,1,1,1]) #np.zeros((batch_size),dtype=np.int32) #[None, self.sequence_length]
